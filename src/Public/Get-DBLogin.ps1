Function Get-DBLogin {
    ...
    begin {
        ...
        DATA LoginQuery {
            @"
SELECT [login].[name] AS [ServerLoginName], ISNULL([role].[name],'public') as [ServerRole]
FROM [sys].[server_principals] login
LEFT OUTER JOIN [sys].[server_role_members] [rm] ON [rm].[member_principal_id] = [login].[principal_id]
LEFT OUTER JOIN [sys].[server_principals] [role] ON [rm].[role_principal_id] = [role].[principal_id]
WHERE [login].[name] = '`$(LoginID)'
"@
        }
        DATA UserQuery {
            @"
SELECT
    [login].[name] AS [ServerLoginName],
    [user].[name] AS [DBUserName],
	[dbrole].[name] AS [DBUserRole]
FROM
    [sys].[server_principals] [login]
    LEFT JOIN
    [sys].[database_principals] [user] ON [login].[sid] = [user].[sid]
	LEFT OUTER JOIN
	[sys].[database_role_members] AS [dbrolemembers]
		ON [dbrolemembers].[member_principal_id] = [user].[principal_id]
	LEFT OUTER JOIN [sys].[database_principals] AS [dbrole]
		ON [dbrolemembers].[role_principal_id] = [dbrole].[principal_id]
WHERE
    [login].[name] = '`$(LoginID)'
"@
        }
        ...
        #endregion
        $Variable = "LoginID=$Login"
        $PSBoundParameters.Remove('Login') | Out-Null
        $Query = $(if ($PSBoundParameters.ContainsKey('Database')) {
                $UserQuery
            } else {
                $LoginQuery
            })
