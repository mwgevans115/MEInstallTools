...
###############################################################################
. $PSScriptRoot\_build.functions.ps1
...
Task FixManifest {
    $ManifestFileContent = Get-Content $ManifestFile
    $Update = $false
    If ($ManifestFileContent.Contains("CmdletsToExport = '*'")){
        $ManifestFileContent = $ManifestFileContent.Replace("CmdletsToExport = '*'","CmdletsToExport = @()")
        $Update = $true
    }
    If ($ManifestFileContent.Contains("AliasesToExport = '*'")){
        $ManifestFileContent = $ManifestFileContent.Replace("AliasesToExport = '*'","AliasesToExport = @()")
        $Update = $true
    }
    If ($Update) {
        Set-Content -Path $ManifestFile -Value $ManifestFileContent -Force
        if (Test-GitRepo) {
            git add $ManifestFile
            git commit -m "[Update] Removed Wildcards from CmdletsToExport and AliasesToExport"
        }
    }
}

Task InitGit -depends FixManifest {
    if (Test-Path -Path $ProjectRoot\.gitignore) {
        if (Test-GitRepo) {
            Write-Verbose "$($psake.context.currentTaskName) - $ProjectRoot is already Git Repository"
        }
        else {
            New-GitRepo
        }
...
Task Init -depends InitGit -requiredVariables OutDir {    if (!(Test-Path -LiteralPath $OutDir)) {
