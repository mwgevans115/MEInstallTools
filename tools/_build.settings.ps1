...
Properties {
    ...
    [System.Diagnostics.CodeAnalysis.SuppressMessage('PSUseDeclaredVarsMoreThanAssigments', '')]
    $ManifestFile = Get-Item $SrcRootDir/*.psd1 |
    Where-Object { $null -ne (Test-ModuleManifest -Path $_ -ErrorAction SilentlyContinue) } |
    Select-Object -First 1
    $ModuleName = $ManifestFile | Foreach-Object BaseName
    ...
    # ------------------------- Build Versioning    ---------------------------
    # Automatic Versioning always updates build version updates minor or major
    # Depending on changes to module detected
    [System.Diagnostics.CodeAnalysis.SuppressMessage('PSUseDeclaredVarsMoreThanAssigments', '')]
    $IncrementVersion = $true
    if (!(Test-AllChangesCommitted)) {
        Write-Warning "Changes Not Commited, not updating version"
        [System.Diagnostics.CodeAnalysis.SuppressMessage('PSUseDeclaredVarsMoreThanAssigments', '')]
