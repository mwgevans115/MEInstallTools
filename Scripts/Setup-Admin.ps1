...
param (
    ...
    [Parameter(HelpMessage = "Admin Aplication Folder")]
    $AdminFolder = "oms-admin",
    [Parameter(HelpMessage = "Admin DNS Name")]
    $AdminDNSName = "localhost",
    ...
    $AdminDocumentFolder = 'Shared Documents\Latest\Admin',
...
$ApplicationPath = Join-Path $WebRootPath $AdminFolder
...
Write-Log -Level INFO -Message "Downloading {0} from {1}" -Arguments $AdminDocumentFolder, $URL
$Downloads = Get-SharepointFolder -SiteURI $URL -DocumentFolder $AdminDocumentFolder -UseWebAuth
...
Get-ChildItem -Path (Join-Path $TargetPath '*') -Recurse -Include 'oms-admin' | ForEach-Object {
