...
param (
    ...
    [Parameter(HelpMessage = "Sharepoint location for Payment Gateway")]
    $PaymentGatewayDocumentFolder = 'Shared Documents\Latest\Payment Gateway'
)
...
$BackupArchive = Join-Path $SoftwareBackupPath "$(Split-Path $PaymentGatewaySoftwarePath -Leaf) $(get-date -Format "yyyy-MM-dd HHmm").7z"
$BackupChanges = Join-Path $SoftwareBackupPath "$scriptName $(get-date -Format "yyyy-MM-dd HHmm").7z"
if (Get-ChildItem -Path $PaymentGatewaySoftwarePath -File -Recurse) {
    Write-Log -Level WARNING 'Backing up {0} to {1}' -Arguments $PaymentGatewaySoftwarePath, $BackupArchive
    Compress-7Zip -ArchiveFileName $BackupArchive -Path $PaymentGatewaySoftwarePath
}
#endregion
...
$PreRequisiteFolder = Join-Path $PaymentGatewaySoftwarePath 'PreRequisites'
...
$PreRequisites | ForEach-Object {
    ...
    If (Test-Path (Join-Path $PreRequisiteFolder $_.Name)) {
        If ([Version]((Get-Version $_).Version) -gt
            ...
            Compress-7Zip -ArchiveFileName $BackupChanges -Path (Join-Path $PaymentGatewaySoftwarePath $_.Name) -PreserveDirectoryRoot
...
Write-Log -Level INFO -Message "Downloading {0} from {1}" -Arguments $PaymentGatewayDocumentFolder, $URL
$Downloads = Get-SharepointFolder -SiteURI $URL -DocumentFolder $PaymentGatewayDocumentFolder -UseWebAuth
...
$TempBackupFolder = New-TempFolder (Split-Path $PaymentGatewaySoftwarePath -Leaf)
[System.Collections.ArrayList]$fileList = @()
Get-ChildItem -Path (Join-Path $TargetPath '*') -Recurse -Include 'Software' | ForEach-Object {
    $SourcePath = $_.FullName
    Get-ChildItem -Path $SourcePath -Recurse -File | ForEach-Object {
        $SourceFile = $_.FullName
        $TargetFile = $SourceFile.Replace($SourcePath, $PaymentGatewaySoftwarePath)
